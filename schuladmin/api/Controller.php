...
// Teacher part of the controller
        if ($ressource == 'lehrer') {
            ...
            switch ($method) {
                case 'GET':
                    return formatMessage(200, $fach->db_get_fach());
                case 'POST':
                    return insertFach($dataFromClient);
                case 'PUT':
                    return updateFach($dataFromClient);
                case 'DELETE':
                    return formatMessage(200, deleteFach($dataFromClient));
                case 'OPTIONS':
                    return formatMessage(405);
                default:
                    return formatMessage(405);
            ...
            switch ($method) {
                case 'GET':
                    if (!empty($id)) {
                        ...
                        return formatMessage(200, getKlasse());
                    }
                case 'POST':
                    return insertKlasse($dataFromClient);
                case 'PUT':
...
// INSERT ----------------------------------------------------------
    ...
    {
        $status = checkInputClass($class);
        if ($status['status'] == 200) {
            db_insert_klasse($class);
        }
        return $status;
    }
